...
#[test]
fn second_configuration_editor_waits_for_the_first_to_finish() {
    let mut f = fixture("two-admins"); let g = &mut f.game;
    ...
    assert!(mud_game::olc::cedit::cedit_parse(g, a, olc, b"y").is_none());
    mud_game::olc::cedit::do_oasis_cedit(g, second, b"", 0, 0);
    assert_eq!(g.descriptors.get(b).unwrap().state, ConState::Cedit);
    assert_eq!(g.olc[&b].config.as_ref().unwrap().ok, b"Changed!\r\n");
}
