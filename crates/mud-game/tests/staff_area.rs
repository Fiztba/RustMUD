...
fn use_area_item(kind: i32, label: &str) -> Vec<i32> {
    ...
    obj.type_flag = kind;
    obj.values = [1, 5, 5, mud_data::spells::SPELL_EARTHQUAKE];
    ...
    targets.into_iter().map(|ch| g.ch(ch).points.hit).collect()
}

#[test]
fn staff_area_spell_hits_each_target_once_per_charge() {
    let wand = use_area_item(flags::ITEM_WAND, "wand");
    let staff = use_area_item(flags::ITEM_STAFF, "staff");
